...
public class ClientInfo1AHelper implements ISerializeHelper
{
	...
	ISerializeHelper _clientInfo0Ah = new ClientInfo0AHelper ();
	...
	public Object createChannelObj(DataInputStream dis) throws IOException
	{
		...
		ClientInfo[] retArray = new ClientInfo[length];
		for (int i = 0; i < length; i++)
			retArray[i]=(ClientInfo)_clientInfo0Ah.createChannelObj(dis);
		return retArray; 
	}
	...
	public void serialChannelObj(Object toArray, DataOutputStream dos) throws IOException
	{
		...
		for (int i=0;i<st.length;i++)
			_clientInfo0Ah.serialChannelObj(st[i], dos);
	}
