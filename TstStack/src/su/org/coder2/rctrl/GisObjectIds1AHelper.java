...
public class GisObjectIds1AHelper implements ISerializeHelper
{
	...
	ISerializeHelper _gisObjectIds0Ah = new GisObjectIds0AHelper ();
	...
	public Object createChannelObj(DataInputStream dis) throws IOException
	{
		...
		GisObjectIds[] retArray = new GisObjectIds[length];
		for (int i = 0; i < length; i++)
			retArray[i]=(GisObjectIds)_gisObjectIds0Ah.createChannelObj(dis);
		return retArray; 
	}
	...
	public void serialChannelObj(Object toArray, DataOutputStream dos) throws IOException
	{
		...
		for (int i=0;i<st.length;i++)
			_gisObjectIds0Ah.serialChannelObj(st[i], dos);
	}
