...
public class double2AHelper implements ISerializeHelper
{
	...
	ISerializeHelper _double1Ah = new double1AHelper ();
	...
	public Object createChannelObj(DataInputStream dis) throws IOException
	{
		...
		for (int i = 0; i < length; i++)
			retArray[i]=(double[])_double1Ah.createChannelObj(dis);
		return retArray; 
	}
	...
	public void serialChannelObj(Object toArray, DataOutputStream dos) throws IOException
	{
		...
		dos.writeByte(0);
		double[][] st=(double[][]) toArray;
		dos.write(SerialUtils.serialint32(st.length));
		for (int i=0;i<st.length;i++)
			_double1Ah.serialChannelObj(st[i], dos);
	}

	public String toPrintableString(Object toPrintable)
	{
		if (toPrintable instanceof double[][])
		{
			String retVal = "";
			double[][] st=(double[][]) toPrintable;
			for (int i=0;i<st.length;i++)
				retVal+=_double1Ah.toPrintableString(st[i]);
			return retVal;
		}
