...
public class int2AHelper implements ISerializeHelper
{
	...
	ISerializeHelper _int1Ah = new int1AHelper ();
	...
	public Object createChannelObj(DataInputStream dis) throws IOException
	{
		...
		for (int i = 0; i < length; i++)
			retArray[i]=(int[])_int1Ah.createChannelObj(dis);
		return retArray; 
	}
	...
	public void serialChannelObj(Object toArray, DataOutputStream dos) throws IOException
	{
		...
		for (int i=0;i<st.length;i++)
			_int1Ah.serialChannelObj(st[i], dos);
	}

	public String toPrintableString(Object toPrintable)
	{
		if (toPrintable instanceof int[][])
		{
			String retVal = "";
			int[][] st=(int[][]) toPrintable;
			for (int i=0;i<st.length;i++)
				retVal+=_int1Ah.toPrintableString(st[i]);
			return retVal;
		}
