...
public class PairOptions2AHelper implements ISerializeHelper
{
	...
	ISerializeHelper _pairOptions1Ah = new PairOptions1AHelper ();
	...
	public Object createChannelObj(DataInputStream dis) throws IOException
	{
		...
		PairOptions[][] retArray = new PairOptions[length][];
		for (int i = 0; i < length; i++)
			retArray[i]=(PairOptions[])_pairOptions1Ah.createChannelObj(dis);
		return retArray; 
	}
	...
	public void serialChannelObj(Object toArray, DataOutputStream dos) throws IOException
	{
		...
		for (int i=0;i<st.length;i++)
			_pairOptions1Ah.serialChannelObj(st[i], dos);
	}
	...
	public String toPrintableString(Object toPrintable)
	{
		if (toPrintable instanceof PairOptions[][])
		{
			...
			for (int i=0;i<st.length;i++)
				retVal+=_pairOptions1Ah.toPrintableString(st[i]);
			return retVal;
		}
