...
public class GisObjectPair1AHelper implements ISerializeHelper
{
	...
	ISerializeHelper _gisObjectPair0Ah = new GisObjectPair0AHelper ();
	...
	public Object createChannelObj(DataInputStream dis) throws IOException
	{
		...
		GisObjectPair[] retArray = new GisObjectPair[length];
		for (int i = 0; i < length; i++)
			retArray[i]=(GisObjectPair)_gisObjectPair0Ah.createChannelObj(dis);
		return retArray; 
	}
	...
	public void serialChannelObj(Object toArray, DataOutputStream dos) throws IOException
	{
		...
		for (int i=0;i<st.length;i++)
			_gisObjectPair0Ah.serialChannelObj(st[i], dos);
	}
