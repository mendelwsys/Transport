...
public class CheckTransl
{
	...
	public void chekItTransl2(DataInputStream dis,String args[])
	{
		try
		{
			...
			{
				if (lst.get(i).equals("long") && i<lst.size()-1 && lst.get(i+1).equals("long"))
				{
					lst.remove(i);
					lst.remove(i);
					lst.add(i,"longlong");
	...
	public void chekItTransl1()
	{
		try
		{
			...
			DataInputStream dis = new DataInputStream(new ByteArrayInputStream(instr.getBytes(Coder.getcodercharset())));
