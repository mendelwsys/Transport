 ...
 * Date: 01.02.2005
 * Time: 13:47:24
 * Базовый класс для преобразования double
 */
public class Double0AHelper
		implements ISerializeHelper
{
	...
	public Object createChannelObj(DataInputStream dis) throws IOException
	{
		...
		byte[] bt=new byte[8];
		dis.readFully(bt);
		return new Double(SerialUtils.unserializedbl64(bt));
	}
	...
	public void serialChannelObj(Object toArray, DataOutputStream dos) throws IOException
	{
		...
		dos.writeByte(0);
		byte[] bt=SerialUtils.serializedbl64(((Double) toArray).doubleValue());
		dos.write(bt);
	}
	...
	public String toPrintableString(Object toPrintable)
	{
		if (toPrintable instanceof Double)
			return ((Double) toPrintable).toString() + "\n";
